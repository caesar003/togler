...
print_help() {
  ...
  echo "        (Wayland: auto-installs GNOME Shell extension on first use)"
  echo
  ...
  echo
  echo "  togler --setup-wayland"
  echo "        Manually install GNOME Shell extension for Wayland support"
  echo "        (Usually done automatically on first use)"
  echo
  echo "Platform Support:"
  echo "  X11:      Uses xdotool (install: sudo apt install xdotool)"
  echo "  Wayland:  Uses GNOME Shell extension (auto-installed)"
  echo
  echo "Examples:"
  echo "  togler -t code"
...
toggle_wayland() {
  ...
  gdbus call --session \
    ...
    --object-path /org/gnome/Shell/Extensions/Togler \
    --method org.gnome.Shell.Extensions.Togler.ToggleByWmClass \
    "$1" >/dev/null 2>&1
}
#}}}
...
toggle() {
  ...
  if is_wayland; then
    ...
    if toggle_wayland "$window_class"; then
      exit 0
    else
      log_error "Failed to toggle via extension"
      echo "💡 Make sure the extension is enabled and GNOME Shell has been restarted"
      exit 1
    fi
